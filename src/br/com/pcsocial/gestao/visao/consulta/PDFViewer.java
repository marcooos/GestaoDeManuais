...
public class PDFViewer extends JFrame implements KeyListener,
	 ...
	 */
	/*
	 * public Icon getIcon(String name) { Icon icon = null; URL url = null; try
	 * { url = getClass().getResource(name);
	 * 
	 * icon = new ImageIcon(url); System.out.println("Couldn't find " + url); }
	 * catch (Exception e) { System.out.println("Couldn't find " +
	 * getClass().getName() + "/" + name); e.printStackTrace(); } return icon; }
	 */
	...
	Action printAction = new AbstractAction("Print...", new ImageIcon(
			getClass().getResource("/gui/gfx/print.gif"))) {
		/**
		 * 
		 */
		private static final long serialVersionUID = 1L;
	...
	ZoomAction zoomInAction = new ZoomAction("Zoom in", new ImageIcon(
			getClass().getResource("/gui/gfx/zoomin.gif")), 2.0);
	ZoomAction zoomOutAction = new ZoomAction("Zoom out", new ImageIcon(
			getClass().getResource("/gui/gfx/zoomout.gif")), 0.5);
	@SuppressWarnings("serial")
	Action zoomToolAction = new AbstractAction("", new ImageIcon(getClass()
			.getResource("/gui/gfx/zoom.gif"))) {
	...
	Action fullScreenAction = new AbstractAction("Full screen", new ImageIcon(
			getClass().getResource("/gui/gfx/fullscrn.gif"))) {

		public void actionPerformed(ActionEvent evt) {
			doFullScreen((evt.getModifiers() & evt.SHIFT_MASK) != 0);
		}
	};
	Action nextAction = new AbstractAction("Next", new ImageIcon(getClass()
			.getResource("/gui/gfx/next.gif"))) {

		public void actionPerformed(ActionEvent evt) {
			doNext();
		}
	};
	Action firstAction = new AbstractAction("First", new ImageIcon(getClass()
			.getResource("/gui/gfx/first.gif"))) {

		public void actionPerformed(ActionEvent evt) {
			doFirst();
		}
	};
	Action lastAction = new AbstractAction("Last", new ImageIcon(getClass()
			.getResource("/gui/gfx/last.gif"))) {

		public void actionPerformed(ActionEvent evt) {
			doLast();
		}
	};
	Action prevAction = new AbstractAction("Prev", new ImageIcon(getClass()
			.getResource("/gui/gfx/prev.gif"))) {
	...
	protected void init() {
		...
		file.add(quitAction);
		mb.add(file);
		JMenu view = new JMenu("Visualização");
	 ...
	 */
	public void openFile(URL url, int pagina) throws IOException {
		...
		buf.put(byteBuf);
		openPDFByteBuffer(buf, url.toString(), url.getFile(), pagina);
	}
	 ...
	 * @throws IOException
	 */
	public void openFile(File file, int pagina) throws IOException {
		...
		ByteBuffer buf = channel.map(FileChannel.MapMode.READ_ONLY, 0,
				channel.size());
		openPDFByteBuffer(buf, file.getPath(), file.getName(), pagina);
	}
	 ...
	 *            the file to open
	 */
	public void openFileUnMapped(File file, int pagina) throws IOException {
	 ...
	 */
	/*
	 * public void doOpen() { try { JFileChooser fc = new JFileChooser();
	 * fc.setCurrentDirectory(prevDirChoice); fc.setFileFilter(pdfFilter);
	 * fc.setMultiSelectionEnabled(false); int returnVal =
	 * fc.showOpenDialog(this); if (returnVal == JFileChooser.APPROVE_OPTION) {
	 * try { prevDirChoice = fc.getSelectedFile();
	 * openFile(fc.getSelectedFile()); } catch (IOException ioe) {
	 * ioe.printStackTrace(); } } } catch (Exception e) {
	 * JOptionPane.showMessageDialog(split, "Opening files from your local " +
	 * "disk is not available\nfrom the " + "Java Web Start version of this " +
	 ...
	 */
	public void doOpen(String name, int pagina) {
		try {
			URL url = new URL(name);
			openFile(new URL(name), pagina);
		} catch (IOException ioe) {
			try {
				openFile(new File(name), pagina);
			} catch (IOException ex) {
				Logger.getLogger(PDFViewer.class.getName()).log(Level.SEVERE,
						null, ex);
			}
		}
	}
	...
	public void iniciarInterface(Long id) {
		...
		File file = new File(documento.getArquivo());
		file = conversores.convertByteToFile(documento.getAnexo(), documento.getArquivo());		
		try {
			openFile(file, indices.getPagina()-1);
			file.delete();
		} catch (IOException e) {
